...
fn read_cpu_usage_from_job_object() -> Option<CpuStats> {
    // SAFETY: `info` is a stack-allocated `JOBOBJECT_BASIC_ACCOUNTING_INFORMATION` initialized via `zeroed()`, so the compiler guarantees its alignment.
    ...
    // Passing a null handle for the job handle is documented to use the current process's job object.
    let result = unsafe {
...
mod tests {
    ...
    #[test]
    fn test_mul_overflow_returns_none() {
