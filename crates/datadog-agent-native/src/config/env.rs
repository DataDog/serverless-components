...
//!
//! This module provides the ability to configure the Datadog agent using environment
//! variables, following Datadog's standard naming conventions (DD_* prefix).
//!
//! # Configuration Priority
//!
//! Environment variables are loaded via the Figment configuration system and can be
//! overridden by other configuration sources (YAML files, defaults, etc.) depending
//! on the merge strategy.
//!
//! # Supported Environment Variables
//!
//! This module supports all environment variables defined in the `EnvConfig` struct,
//! including:
...
//! - `DD_TAGS`: Additional tags (comma-separated key:value pairs)
//! - Many more trace, log, and metrics configuration options
//!
//! See `EnvConfig` struct for the complete list of supported variables.
...
pub struct EnvConfig {
    ...
    ///
    /// Enable enhanced metrics. Default is `true`.
    #[serde(deserialize_with = "deserialize_optional_bool_from_anything")]
    pub enhanced_metrics: Option<bool>,
    /// @env `DD_COMPUTE_TRACE_STATS_ON_EXTENSION`
    ///
    /// If true, enable computation of trace stats on the agent side.
    ...
    ///
    /// Enable Remote Configuration. Default is `true`.
    #[serde(deserialize_with = "deserialize_optional_bool_from_anything")]
    pub remote_configuration_enabled: Option<bool>,
    /// @env `DD_REMOTE_CONFIGURATION_API_KEY`
    ///
    /// Dedicated API key for Remote Configuration (falls back to DD_API_KEY).
    pub remote_configuration_api_key: Option<String>,
    /// @env `DD_REMOTE_CONFIGURATION_KEY`
    ///
    /// Legacy Remote Configuration key.
    pub remote_configuration_key: Option<String>,
    /// @env `DD_REMOTE_CONFIGURATION_NO_TLS`
    ///
    /// Disable TLS for Remote Configuration. Default is `false`.
    #[serde(deserialize_with = "deserialize_optional_bool_from_anything")]
    pub remote_configuration_no_tls: Option<bool>,
    /// @env `DD_REMOTE_CONFIGURATION_NO_TLS_VALIDATION`
    ///
    /// Skip TLS validation for Remote Configuration. Default is `false`.
    #[serde(deserialize_with = "deserialize_optional_bool_from_anything")]
    ...
    /// Valid values:
    /// - `http_fixed_port` (or `fixed`, `http`): HTTP servers on fixed ports (default)
    /// - `http_ephemeral_port` (or `ephemeral`, `dynamic`): HTTP servers on auto-assigned ports
    /// - `ffi_only` (or `ffi`, `no_http`): No HTTP servers, FFI submission only
    #[serde(deserialize_with = "deserialize_optional_string")]
    ...
    /// - Default: 8126 for fixed port mode
    /// - Set to 0 for ephemeral (auto-assigned) port
    /// - Ignored in FFI-only mode
    #[serde(deserialize_with = "deserialize_option_lossless")]
    pub trace_agent_port: Option<u16>,

    /// @env `DD_TRACE_AGENT_UDS_PATH`
    ///
    /// Unix socket path for the trace agent HTTP server.
    /// - Only used when operational_mode is HttpUds
    /// - If not set, path will be auto-generated using PID (e.g., /tmp/dd-trace-12345.sock)
    /// - Ignored in other operational modes
    #[serde(deserialize_with = "deserialize_optional_string")]
    pub trace_agent_uds_path: Option<String>,

    /// @env `DD_TRACE_AGENT_UDS_PERMISSIONS`
    ///
    /// Permissions for the Unix socket file (octal format).
    /// - Default: 0o666 (read/write for all)
    /// - Example: 0o660 for read/write by owner and group only
    /// - Only used when operational_mode is HttpUds
    #[serde(deserialize_with = "deserialize_option_lossless")]
    pub trace_agent_uds_permissions: Option<u32>,

    /// @env `DD_METRICS_AGENT_PORT`
    ///
    /// Port for the metrics agent (DogStatsD) server.
    /// - Default: 8125 for fixed port mode
    /// - Set to 0 for ephemeral (auto-assigned) port
    /// - Ignored in FFI-only mode
    #[serde(deserialize_with = "deserialize_option_lossless")]
    pub metrics_agent_port: Option<u16>,

    /// @env `DD_LOGS_AGENT_PORT`
    ///
    /// Port for the logs agent HTTP server.
    /// - Default: (if implemented)
    /// - Set to 0 for ephemeral (auto-assigned) port
    /// - Ignored in FFI-only mode
    #[serde(deserialize_with = "deserialize_option_lossless")]
    ...
    /// @env `DD_DOGSTATSD_ENABLED`
    ///
    /// Enable the DogStatsD UDP server for receiving StatsD metrics on 127.0.0.1.
    /// - Default: false
    #[serde(deserialize_with = "deserialize_optional_bool_from_anything")]
...
fn merge_config(config: &mut Config, env_config: &EnvConfig) {
    ...
    // Remote Configuration
    merge_option_to_value!(config, env_config, remote_configuration_enabled);
    merge_option!(config, env_config, remote_configuration_api_key);
    merge_option!(config, env_config, remote_configuration_key);
    merge_option_to_value!(config, env_config, remote_configuration_no_tls);
    merge_option_to_value!(config, env_config, remote_configuration_no_tls_validation);

    // Operational Mode
    if let Some(mode_str) = &env_config.agent_mode {
        if let Some(mode) = crate::config::operational_mode::OperationalMode::from_env_str(mode_str)
        {
            config.operational_mode = mode;
        } else {
            tracing::error!("Invalid DD_AGENT_MODE value '{}', using default", mode_str);
        }
    }
    merge_option!(config, env_config, trace_agent_port);
    merge_option!(config, env_config, trace_agent_uds_path);
    merge_option_to_value!(config, env_config, trace_agent_uds_permissions);
    merge_option!(config, env_config, metrics_agent_port);
    merge_option!(config, env_config, logs_agent_port);

    // DogStatsD Configuration
    merge_option_to_value!(config, env_config, dogstatsd_enabled);
    merge_option_to_value!(config, env_config, dogstatsd_port);
}
...
mod tests {
    ...
    fn test_merge_config_overrides_with_environment_variables() {
        figment::Jail::expect_with(|jail| {
            ...
            // Operational Mode
            jail.set_env("DD_AGENT_MODE", "http_ephemeral_port");
            jail.set_env("DD_TRACE_AGENT_PORT", "9999");
            jail.set_env("DD_METRICS_AGENT_PORT", "8888");
            jail.set_env("DD_LOGS_AGENT_PORT", "7777");
            ...
            let expected_config = Config {
                ...
                // DogStatsD Configuration - using defaults since no env vars set for it in this test
    ...
    #[test]
    fn test_invalid_agent_mode_defaults_to_fixed_port() {
        figment::Jail::expect_with(|jail| {
            jail.clear_env();
            jail.set_env("DD_API_KEY", "test-key");
            jail.set_env("DD_AGENT_MODE", "invalid_mode_value");
            ...
            // Should default to HttpFixedPort when invalid value provided
    ...
    #[test]
    fn test_agent_mode_http_fixed_port() {
        figment::Jail::expect_with(|jail| {
            jail.clear_env();
            jail.set_env("DD_API_KEY", "test-key");
            jail.set_env("DD_AGENT_MODE", "http_fixed_port");
            jail.set_env("DD_TRACE_AGENT_PORT", "8126");
    ...
    #[test]
    fn test_agent_mode_aliases() {
        figment::Jail::expect_with(|jail| {
            // Test "fixed" alias
            jail.clear_env();
            jail.set_env("DD_API_KEY", "test-key");
            jail.set_env("DD_AGENT_MODE", "fixed");
            ...
            jail.set_env("DD_AGENT_MODE", "ephemeral");
            let mut config2 = Config::default();
            env_config_source
                .load(&mut config2)
                .expect("Failed to load config");

            assert_eq!(config2.operational_mode, OperationalMode::HttpEphemeralPort);

            // Test "uds" alias
            jail.set_env("DD_AGENT_MODE", "uds");
            let mut config3 = Config::default();
            env_config_source
                .load(&mut config3)
                .expect("Failed to load config");

            assert_eq!(config3.operational_mode, OperationalMode::HttpUds);
    ...
    #[test]
    fn test_empty_agent_mode_uses_default() {
        figment::Jail::expect_with(|jail| {
            jail.clear_env();
            jail.set_env("DD_API_KEY", "test-key");
            jail.set_env("DD_AGENT_MODE", ""); // Empty string
            ...
            // Empty string should be treated as not set, use default
            assert_eq!(config.operational_mode, OperationalMode::HttpFixedPort);
    ...
    #[test]
    fn test_whitespace_agent_mode_uses_default() {
        figment::Jail::expect_with(|jail| {
            jail.clear_env();
            jail.set_env("DD_API_KEY", "test-key");
            jail.set_env("DD_AGENT_MODE", "   "); // Whitespace only
            ...
            // Whitespace should default to HttpFixedPort
            assert_eq!(config.operational_mode, OperationalMode::HttpFixedPort);
    ...
    #[test]
    fn test_port_validation_max_value() {
        figment::Jail::expect_with(|jail| {
            jail.clear_env();
            jail.set_env("DD_API_KEY", "test-key");
            jail.set_env("DD_TRACE_AGENT_PORT", "65535"); // Max valid port
            ...
            assert_eq!(config.trace_agent_port, Some(65535));
    ...
    #[test]
    fn test_port_validation_port_zero() {
        figment::Jail::expect_with(|jail| {
            jail.clear_env();
            jail.set_env("DD_API_KEY", "test-key");
            jail.set_env("DD_TRACE_AGENT_PORT", "0"); // Port 0 is valid (ephemeral)
    ...
    fn test_invalid_port_ignored() {
        figment::Jail::expect_with(|jail| {
            jail.clear_env();
            jail.set_env("DD_API_KEY", "test-key");
            jail.set_env("DD_TRACE_AGENT_PORT", "invalid"); // Invalid port
            ...
            // Invalid port should be ignored, use None
    ...
    #[test]
    fn test_negative_port_ignored() {
        figment::Jail::expect_with(|jail| {
            jail.clear_env();
            jail.set_env("DD_API_KEY", "test-key");
            jail.set_env("DD_TRACE_AGENT_PORT", "-1"); // Negative port
            ...
            // Negative port should be ignored
            assert_eq!(config.trace_agent_port, None);
    ...
    #[test]
    fn test_port_above_max_ignored() {
        figment::Jail::expect_with(|jail| {
            jail.clear_env();
            jail.set_env("DD_API_KEY", "test-key");
            jail.set_env("DD_TRACE_AGENT_PORT", "65536"); // Above max port
            ...
            // Port above 65535 should be ignored
            assert_eq!(config.trace_agent_port, None);
    ...
    #[test]
    fn test_dogstatsd_config_defaults() {
        figment::Jail::expect_with(|jail| {
            jail.clear_env();
            jail.set_env("DD_API_KEY", "test-key");
            ...
            // DogStatsD should be disabled by default
            assert_eq!(config.dogstatsd_enabled, false);
    ...
    #[test]
    fn test_dogstatsd_enabled_from_env() {
        figment::Jail::expect_with(|jail| {
            jail.clear_env();
            jail.set_env("DD_API_KEY", "test-key");
            jail.set_env("DD_DOGSTATSD_ENABLED", "true");
            ...
            assert_eq!(config.dogstatsd_enabled, true);
    ...
    #[test]
    fn test_dogstatsd_port_from_env() {
        figment::Jail::expect_with(|jail| {
            jail.clear_env();
            jail.set_env("DD_API_KEY", "test-key");
            jail.set_env("DD_DOGSTATSD_PORT", "8126");
            ...
            assert_eq!(config.dogstatsd_port, 8126);
    ...
    #[test]
    fn test_dogstatsd_all_config_from_env() {
        figment::Jail::expect_with(|jail| {
            jail.clear_env();
            jail.set_env("DD_API_KEY", "test-key");
            jail.set_env("DD_DOGSTATSD_ENABLED", "true");
            jail.set_env("DD_DOGSTATSD_PORT", "9125");
            ...
            assert_eq!(config.dogstatsd_enabled, true);
            assert_eq!(config.dogstatsd_port, 9125);
    ...
    #[test]
    fn test_dogstatsd_enabled_case_insensitive() {
        figment::Jail::expect_with(|jail| {
            jail.clear_env();
            jail.set_env("DD_API_KEY", "test-key");
            jail.set_env("DD_DOGSTATSD_ENABLED", "TRUE");
