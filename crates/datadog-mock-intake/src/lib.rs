...
//! In-process mock Datadog intake for APM payload-level integration tests.
//!
//! Spawns a hyper server on a local port that accepts the same APM endpoints
//! serverless components flush to, decodes msgpack / protobuf payloads on
//! arrival, and stores the decoded structs. Callers then use typed query
...
//!
//! Every completed POST attempt to one of these endpoints, including decode
//! failures and failure-injection rejections, is also recorded as a raw
...
//! ([`PipelineStatsPayload`] below); serde ignores the rest of the wire
//! payload, including the `serde_bytes` latency sketches.
...
use hyper::header;
use hyper::{Method, Request, Response, StatusCode, service::service_fn};
...
/// Maximum accepted request body size, matching the 2 MiB default extractor
/// limit the intake was originally served with. Enforced on the wire body,
...
/// Options controlling a mock-intake server. The default
/// (`MockIntakeOptions::default()`) disables all diagnostics.
#[derive(Clone, Debug, Default)]
pub struct MockIntakeOptions {
    /// Port to bind on `127.0.0.1`. `0` lets the OS assign a free port; the
    /// actual address is available via `MockIntake::base_url()`.
    pub port: u16,
...
/// Errors from `MockIntake::start_with_options`.
#[derive(Debug, thiserror::Error)]
pub enum MockIntakeError {
    #[error("mock_intake: failed to bind listener on 127.0.0.1:{port}: {source}")]
    Bind {
        port: u16,
        #[source]
        source: std::io::Error,
    ...
    #[error(
        "mock_intake: failed to initialize dump directory {}: {source}",
        .dir.display()
    )]
    DumpDir {
        dir: PathBuf,
...
/// via `MockIntake::stats_payloads()` / `trace_payloads()` and friends.
#[derive(Debug)]
struct SharedState {
...
/// The APM endpoints the intake decodes.
...
impl Endpoint {
    fn from_path(path: &str) -> Option<Self> {
        match path {
            "/api/v0.2/stats" => Some(Self::Stats),
            "/api/v0.2/traces" => Some(Self::Traces),
            "/api/v0.1/pipeline_stats" => Some(Self::PipelineStats),
            _ => None,
        }
    }
}

/// A running mock-intake server. Drop shuts it down.
#[derive(Debug)]
...
impl MockIntake {
    ...
    pub async fn start() -> Self {
        Self::start_with_options(MockIntakeOptions::default())
            .await
            .expect("mock_intake: failed to start server on 127.0.0.1:0")
    }
    ...
    pub async fn start_with_options(options: MockIntakeOptions) -> Result<Self, MockIntakeError> {
        if let Some(dir) = &options.dump_dir {
            std::fs::create_dir_all(dir).map_err(|source| MockIntakeError::DumpDir {
                dir: dir.clone(),
                source,
            })?;
        }
        ...
        let listener = TcpListener::bind(("127.0.0.1", options.port))
            .await
            .map_err(|source| MockIntakeError::Bind {
                port: options.port,
                source,
            })?;
        let addr = listener
    ...
    /// Full URL for the DSM pipeline-stats endpoint.
    #[must_use]
    pub fn pipeline_stats_url(&self) -> String {
        format!("{}/api/v0.1/pipeline_stats", self.base_url)
    }
...
fn response(status: StatusCode) -> Response<Full<Bytes>> {
    Response::builder()
        ...
        .expect("mock_intake: static response construction cannot fail")
}
...
async fn handle_request(
    ...
    if req.method() != Method::POST {
        let mut resp = response(StatusCode::METHOD_NOT_ALLOWED);
        resp.headers_mut()
    ...
    let body = match Limited::new(req.into_body(), MAX_BODY_SIZE).collect().await {
        ...
        Err(err) => {
            ...
            eprintln!("mock_intake: failed to read request body: {err}");
...
fn log_summary(
    ...
    status: StatusCode,
    payload_count: usize,
    groups: Option<&BTreeMap<StatsGroupKey, u64>>,
) {
    let mut line = format!(
        "mock-intake: request {request_id} | POST {endpoint} | encoding={} | status={} | payloads={payload_count}",
...
fn dump_request(
    ...
    let Ok(mut json) = serde_json::to_vec_pretty(&envelope) else {
        eprintln!("mock_intake: failed to serialize dump envelope for request {request_id}");
        return;
    };
    json.push(b'\n');
    ...
    loop {
            ...
            .open(&path)
        {
            ...
            Err(e) if e.kind() == std::io::ErrorKind::AlreadyExists => {
                ...
                if suffix > 1000 {
                    eprintln!(
                        "mock_intake: giving up on dump filename for request {request_id} after 1000 collisions"
                    );
                    return;
                }
            }
            Err(e) => {
                eprintln!("mock_intake: failed to create dump {}: {e}", path.display());
                return;
            }
        }
    }
...
fn stats_hits_by_key(payload: &pb::StatsPayload) -> BTreeMap<StatsGroupKey, u64> {
    let mut groups: BTreeMap<StatsGroupKey, u64> = BTreeMap::new();
...
mod tests {
    ...
    async fn post(base_url: &str, path: &str, encoding: Option<&str>, body: Vec<u8>) -> StatusCode {
        ...
        request
            ...
            .send()
            .await
            .expect("test: request to mock intake failed")
            .status()
    }
    ...
    fn client_payload(env: &str) -> pb::ClientStatsPayload {
        pb::ClientStatsPayload {
            service: "mock-intake-smoke".to_string(),
            env: env.to_string(),
    ...
    /// Serialize a stats payload whose wire size is exactly `target` bytes by
    /// padding the client hostname. Above the msgpack str16 boundary the
    /// encoding grows linearly with the string, so a fixed-point adjustment
    /// converges; fails the test if it cannot hit the target exactly.
    fn stats_payload_wire_of_size(target: usize) -> Vec<u8> {
        let base = stats_payload(
            client_payload("local"),
            1,
            vec![grouped_stats("svc", "GET /a", 1, |_| {})],
        );
        let base_len = rmp_serde::to_vec_named(&base)
            .expect("test: msgpack encode failed")
            .len();
        assert!(
            target > base_len,
            "test: target {} must exceed base payload size {base_len}",
            target
        );

        let mut pad = target - base_len;
        for _ in 0..8 {
            let mut client = client_payload("local");
            client.hostname = "x".repeat(pad);
        ...
        panic!("test: could not build a payload of exactly {target} bytes");
    }

    async fn start_default() -> MockIntake {
        MockIntake::start().await
    }
    ...
    async fn default_startup_binds_port_zero_and_exposes_url_helpers() {
        ...
        assert!(
            intake
                .pipeline_stats_url()
                .ends_with("/api/v0.1/pipeline_stats")
        );
    }
    ...
    #[tokio::test]
    async fn traces_endpoint_decodes_zstd_compressed_protobuf() {
        let intake = start_default().await;
        let payload = pb::AgentPayload {
            env: "local".to_string(),
            tracer_payloads: vec![pb::TracerPayload::default()],
        ...
        let body = zstd::stream::encode_all(payload.encode_to_vec().as_slice(), 0)
            .expect("test: zstd compression failed");
        assert_eq!(
            post(&intake.base_url(), "/api/v0.2/traces", Some("zstd"), body).await,
            StatusCode::ACCEPTED
        );
        let captured = intake.trace_payloads();
        assert_eq!(captured.len(), 1);
    ...
    #[tokio::test]
    async fn malformed_bodies_return_400_and_are_not_typed_captured() {
        let intake = start_default().await;
        for path in [
            "/api/v0.2/stats",
            "/api/v0.2/traces",
        ...
        // Raw capture records completed attempts even when decoding failed.
        for path in [
            "/api/v0.2/stats",
            "/api/v0.2/traces",
            "/api/v0.1/pipeline_stats",
            ...
            assert_eq!(
                intake.requests_for_path(path).len(),
                1,
                "malformed POST on {path} should still be raw-captured"
            );
        }
    }
    ...
    async fn fail_stats_first_n_rejects_exactly_n_then_accepts() {
        ...
        // Rejected attempts are not typed-captured; only the successful retry is.
        let captured = intake.stats_payloads();
        assert_eq!(captured.len(), 1);
        assert_eq!(captured[0].stats[0].stats[0].stats[0].hits, 1);
        // Raw capture records every completed attempt, including rejections.
        assert_eq!(intake.requests_for_path("/api/v0.2/stats").len(), 3);
    }
    ...
    #[tokio::test]
    async fn unknown_path_returns_404_and_is_not_captured() {
        let intake = start_default().await;
        let body = rmp_serde::to_vec_named(&stats_payload(
            client_payload("local"),
            1,
        ...
        .expect("test: msgpack encode failed");
        assert_eq!(
            post(&intake.base_url(), "/api/v2/profile", None, body).await,
            StatusCode::NOT_FOUND
        );
        assert!(intake.stats_payloads().is_empty());
        assert!(intake.trace_payloads().is_empty());
        assert!(intake.pipeline_stats_payloads().is_empty());
        assert!(intake.requests_for_path("/api/v2/profile").is_empty());
    }

    #[tokio::test]
    async fn non_post_methods_on_known_paths_return_405_and_are_not_captured() {
        let intake = start_default().await;
        let client = reqwest::Client::new();
        for path in [
            "/api/v0.2/stats",
            ...
            let response = client
                .get(format!("{}{path}", intake.base_url()))
                .send()
                .await
                .expect("test: request failed");
            assert_eq!(response.status(), reqwest::StatusCode::METHOD_NOT_ALLOWED);
            assert!(
                response.headers().get("allow").is_some_and(|v| v == "POST"),
                "405 response must advertise Allow: POST"
            );
            assert!(intake.requests_for_path(path).is_empty());
        }
        assert!(intake.stats_payloads().is_empty());
        assert!(intake.trace_payloads().is_empty());
        assert!(intake.pipeline_stats_payloads().is_empty());
    ...
    #[tokio::test]
    async fn oversized_bodies_return_413_and_are_not_captured_or_counted() {
        let intake = MockIntake::start_with_options(MockIntakeOptions {
            fail_stats_first_n: 1,
            ..MockIntakeOptions::default()
        })
        ...
        // One byte over the wire-body limit.
        let oversized = vec![0u8; MAX_BODY_SIZE + 1];
        assert_eq!(
            post(&intake.base_url(), "/api/v0.2/stats", None, oversized).await,
            StatusCode::PAYLOAD_TOO_LARGE
        );
        assert!(intake.requests_for_path("/api/v0.2/stats").is_empty());
        assert!(intake.stats_payloads().is_empty());

        // The rejected oversized attempt must not consume the failure-injection
        // budget: the next valid stats request is rejected as attempt 1 of 1.
        let body = rmp_serde::to_vec_named(&stats_payload(
            client_payload("local"),
            1,
            vec![grouped_stats("svc", "GET /a", 1, |_| {})],
    ...
    async fn invalid_options_fail_clearly() {
        ...
        let err = MockIntake::start_with_options(MockIntakeOptions {
            dump_dir: Some(file.path().join("nested")),
            ..MockIntakeOptions::default()
        })
        .await
        .expect_err("dump dir under a file must fail");
        assert!(matches!(err, MockIntakeError::DumpDir { .. }));
        ...
        assert!(matches!(err, MockIntakeError::Bind { .. }));
    }
